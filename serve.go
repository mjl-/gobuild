...
var (
	...
	config = struct {
		...
		LogDir      string `sconf-doc:"Directory to store log files. HTTP access logs are written, one file per day. Leave empty to disable logging."`
	}{
...
func serve(args []string) {
	...
	if *listenHTTP != "" {
		msg += " http " + *listenHTTP
		go func() {
			log.Fatal(http.ListenAndServe(*listenHTTP, handler))
		}()
	}
	if config.HTTPS != nil {
		msg += " https :443"
		...
		go func() {
			log.Fatal(http.Serve(m.Listener(), handler))
		}()
	}
	if *listenAdmin != "" {
