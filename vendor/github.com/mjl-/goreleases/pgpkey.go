...
func init() {
	kr, err := openpgp.ReadArmoredKeyRing(strings.NewReader(armoredSigningKey))
	if err != nil {
		log.Fatalf("parsing signing key for verifying go releases: %v", err)
	}
	signingKey = kr
}

// From https://www.google.com/linuxrepositories/, which references
// https://dl.google.com/linux/linux_signing_key.pub.
// We only use the primary key, it has been used since the first go release
// with a .asc file, go1.8.5.
